<template>
  ...
  <div class="w-full h-full flex items-center justify-between bg-white px-3 md:px-6 gap-2 overflow-hidden">
    ...
    <ul ref="menuRef" class="flex items-center gap-1 sm:gap-3 md:gap-6 list-none m-0 p-0 flex-shrink-0 relative">
      <!-- 循环渲染菜单项 -->
      <li v-for="item in menuList" :key="item.key" :data-key="item.key" @click="handleMenuClick(item)" class="
      ...
      <!-- 独立的选中条 - 实现移动动画 -->
      <div class="absolute bottom-0 h-[3px] bg-[#00a1d6] rounded-t-sm transition-all duration-200 ease-in-out"
        :style="{ left: activeBarLeft + 'px', width: activeBarWidth + 'px' }"></div>
    </ul>

    <!-- 中间搜索框区域 -->
    <div class="flex-1 max-w-[240px] md:max-w-[300px] hidden sm:block">
      <el-input class="nav-search-input" placeholder="搜索视频、动态" clearable size="default">
        <!-- 搜索框后缀图标 -->
        <template #suffix>
          <el-icon>
            <Search />
    ...
    <div class="flex items-center gap-3 sm:gap-5 md:gap-8 flex-shrink-0">
      <!-- 循环渲染统计项 -->
      <div v-for="item in statData" :key="item.label" class="flex flex-col items-center text-center">
        <!-- 统计标题：小屏幕下隐藏 -->
...
const updateActiveBar = () => {
  ...
  nextTick(() => {
    ...
    // 找到当前选中的菜单项DOM元素
    let activeLi = menuRef.value.querySelector(`li[data-key="${activeKey.value}"]`)
    if (activeLi) {
      // 更新选中条的位置和宽度
      ...
      // 如果没有找到匹配的菜单项,就设为主页
      activeLi=menuRef.value.querySelector(`li[data-key=""]`)
      activeBarLeft.value = activeLi.offsetLeft
      activeBarWidth.value = activeLi.offsetWidth
    }
  })
...
// 组件挂载时初始化选中条位置
onMounted(() => {
  const getLastPathParam = () => {
    const pathArray = router.path.split("/");
    // 过滤空字符串（处理路径开头/结尾的 /）
    const validArray = pathArray.filter(item => item);
    // 取最后一个元素，无有效值则返回空
    return validArray.pop() || "";
  };
  const lastParam = getLastPathParam();
  console.log("最后一个/后的内容：", lastParam);
    // 更新内部选中状态
  activeKey.value = lastParam
  // 更新选中条位置（带动画）
  updateActiveBar()
